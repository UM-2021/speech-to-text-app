...
const Sucursales: React.FC = () => {
	...
	const getApprovedSucursales = (suc: any, au: any): [] => {
		return suc.filter((s: any) => {
			let ap = false;
			au.forEach((auditoria: any) => {
				if (auditoria.sucursal === s.id && (auditoria.digefe_aprobada || auditoria.extra_aprobada))
					ap = true;
			});
			return ap;
		});
	...
	const getInCourseSucursales = (suc: any, au: any): [] => {
		return suc.filter((s: any) => {
			let ic = false;
			au.map((auditoria: any) => {
				if (auditoria.sucursal === s.id && !auditoria.finalizada) ic = true;
			});
			return ic;
		});
	};
