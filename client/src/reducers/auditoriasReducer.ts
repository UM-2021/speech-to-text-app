...
export const auditoriaDetailsReducer = (
  state = { auditoria: {} },
  action: any
) => {
  switch (action.type) {
...
export const getAuditoriasReducer = (
  state = { auditorias: [] },
  action: any
) => {
  switch (action.type) {
    case GET_AUDITORIAS_REQUEST:
      return { loading: true };
    case GET_AUDITORIAS_SUCCESS:
      return { loading: false, auditorias: [...action.payload] };
    case GET_AUDITORIAS_FAILED:
